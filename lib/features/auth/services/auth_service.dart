...
class AuthService {
  ...
  static Dio _createDefaultDio() {
    ...
    dio.options = BaseOptions(
      baseUrl: 'https://sitebook-api.example.com/api/v1', // TODO: Replace with actual API URL
    ...
    dio.interceptors.add(LogInterceptor(
      requestBody: false,  // SECURITY: Don't log request bodies (contain passwords)
      responseBody: false, // SECURITY: Don't log response bodies (contain tokens)
      requestHeaders: false, // SECURITY: Don't log headers (contain auth tokens)
  ...
  Future<AuthResponse> register(RegisterRequest request) async {
    try {
      ...
      if (response.statusCode == 201) {
        final authResponse = AuthResponse.fromJson(response.data);
        print('✅ AuthService: Registration successful for ${authResponse.user.email}');
        return authResponse;
      } else {
        throw AuthException('Registration failed with status ${response.statusCode}');
      }
    } on DioException catch (e) {
  ...
  Future<AuthTokens> refreshToken(String refreshToken) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw AuthException('Token refresh failed with status ${response.statusCode}');
      }
    } on DioException catch (e) {
  ...
  Future<User> getCurrentUser(String accessToken) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw AuthException('Failed to fetch user profile with status ${response.statusCode}');
      }
    } on DioException catch (e) {
      ...
      throw AuthException('An unexpected error occurred while fetching user profile');
    }
  }
  ...
  Future<User> updateProfile(String accessToken, Map<String, dynamic> updates) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw AuthException('Profile update failed with status ${response.statusCode}');
      }
    } on DioException catch (e) {
      ...
      throw AuthException('An unexpected error occurred while updating profile');
    }
  }
  ...
  AuthException _handleDioException(DioException e) {
    switch (e.type) {
      ...
      case DioExceptionType.badResponse:
        ...
        if (statusCode == 401) {
          return const AuthException('Invalid credentials', 'INVALID_CREDENTIALS');
          ...
          return const AuthException('Email address is already registered', 'EMAIL_EXISTS');
        } else if (statusCode != null && statusCode >= 500) {
          return const AuthException('Server error. Please try again later.', 'SERVER_ERROR');
        } else {
          return AuthException('Request failed with status $statusCode', 'BAD_RESPONSE');
        }
      ...
      case DioExceptionType.connectionError:
        return const AuthException('Unable to connect to the server. Please check your internet connection.');
      
      case DioExceptionType.unknown:
      default:
        return const AuthException('An unexpected error occurred. Please try again.');
    }
  }
}