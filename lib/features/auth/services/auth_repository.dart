...
class AuthRepository {
  ...
  Future<AuthState> initialize() async {
    try {
      ...
      if (tokens == null || user == null) {
        print('⚠️ AuthRepository: Failed to retrieve stored authentication data');
      ...
      if (tokens.isExpired) {
        ...
        try {
          ...
          await _storageService.updateAccessToken(newTokens.accessToken, newTokens.expiresAt);
          ...
          return const AuthState.unauthenticated('Session expired. Please sign in again.');
        }
      }
      ...
      await _storageService.clearAll();
      return const AuthState.unauthenticated('Error loading authentication state');
    }
  }
  ...
  Future<AuthState> signIn(String email, String password) async {
    try {
      ...
      print('✅ AuthRepository: Sign in successful for ${authResponse.user.email}');
      ...
      return const AuthState.unauthenticated('An unexpected error occurred during sign in');
    }
  }

  /// Sign up with user details
  Future<AuthState> signUp(String name, String email, String password, {String? location}) async {
    try {
      ...
      print('✅ AuthRepository: Sign up successful for ${authResponse.user.email}');
      return AuthState.authenticated(authResponse.user);
      ...
      return const AuthState.unauthenticated('An unexpected error occurred during sign up');
    }
  }
  ...
  Future<AuthState> refreshAccessToken() async {
    try {
      ...
      if (currentTokens == null) {
        ...
        return const AuthState.unauthenticated('No authentication tokens found');
      }
      
      final newTokens = await _authService.refreshToken(currentTokens.refreshToken);
      await _storageService.updateAccessToken(newTokens.accessToken, newTokens.expiresAt);
      ...
      await _storageService.clearAll();
      return AuthState.unauthenticated('Session expired. Please sign in again.');
    } catch (e) {
      print('❌ AuthRepository: Unexpected error during token refresh - $e');
      await _storageService.clearAll();
      return const AuthState.unauthenticated('Authentication error. Please sign in again.');
    }
  }
  ...
  Future<AuthState> updateProfile(Map<String, dynamic> updates) async {
    try {
      ...
      final updatedUser = await _authService.updateProfile(accessToken, updates);
      ...
      if (e.code == 'INVALID_CREDENTIALS' || e.message.toLowerCase().contains('unauthorized')) {
        await _storageService.clearAll();
        return const AuthState.unauthenticated('Session expired. Please sign in again.');
      }
      ...
      if (currentUser != null) {
        return AuthState.authenticated(currentUser).copyWith(errorMessage: e.message);
      }
      return AuthState.unauthenticated(e.message);
  ...
  Future<String?> getAccessToken() async {
    try {
      ...
      if (tokens.isExpiringSoon || tokens.isExpired) {
        print('🔄 AuthRepository: Token expiring soon, refreshing automatically');
