...
class _DatePickerCard extends StatelessWidget {
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (isCheckIn) {
      ...
      firstDate = today;
      lastDate = checkOutDate?.subtract(const Duration(days: 1)) ?? 
                 DateTime(today.year + 1, today.month, today.day);
    } else {
      final minCheckOut = checkInDate?.add(const Duration(days: 1)) ?? 
    ...
    if (selectedDate != null) {
      ...
      if (isCheckIn && checkOutDate != null && selectedDate.isAtOrAfter(checkOutDate!)) {
