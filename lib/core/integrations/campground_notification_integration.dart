...
import '../../shared/repositories/campground_repository_impl.dart';
...
class CampgroundNotificationIntegration {
  ...
  static Future<void> initialize() async {
    try {
      ...
      debugPrint('❌ Failed to initialize campground notification integration: $e');
    }
  }
  ...
  static Future<void> notifyCampgroundAvailable({
    ...
    try {
      ...
      // Log analytics event
      await FirebaseConfig.logEvent('campground_availability_notification', parameters: {
  ...
  static Future<void> notifyMonitoringUpdate({
    ...
    try {
      ...
      debugPrint('🔔 Monitoring update notification sent for ${campground.name}');
  ...
  static Future<void> checkMonitoredCampgroundsAvailability(WidgetRef ref) async {
    try {
      // Get all monitored campgrounds
      final campgrounds = await ref.read(searchResultsProvider.future);
      final monitoredCampgrounds = campgrounds.where((c) => c.isMonitored).toList();
      ...
      debugPrint('🏕️ Checking availability for ${monitoredCampgrounds.length} monitored campgrounds');
  ...
  static Future<bool> _checkCampgroundAvailability(Campground campground) async {
  ...
  static Future<void> sendDailySummary(WidgetRef ref) async {
    try {
      ...
      await NotificationService.showLocalNotification(
        ...
        body: 'You are monitoring $monitoredCount ${monitoredCount == 1 ? 'campground' : 'campgrounds'}. Tap to view details.',
        payload: 'daily_summary',
        channel: 'general_notifications',
      );
      
      // Log analytics event
      await FirebaseConfig.logEvent('daily_summary_notification', parameters: {
  ...
  static Future<void> sendWelcomeNotification() async {
    try {
      await NotificationService.showLocalNotification(
        ...
        body: 'We\'ll notify you when your monitored campgrounds become available. Happy camping!',
        payload: 'welcome',
        channel: 'general_notifications',
      );
