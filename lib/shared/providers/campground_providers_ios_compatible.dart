...
final campgroundRepositoryProvider = Provider(
  (ref) => "Demo Mode - iOS Compatible",
);
...
final campgroundsByStateProvider =
    FutureProvider.family<List<Campground>, String>((ref, state) async {
...
final nearbySearchProvider =
    FutureProvider.family<List<Campground>, Map<String, dynamic>>((
      ref,
      params,
    ) async {
...
final backgroundMonitoringStatusProvider =
    FutureProvider<BackgroundMonitoringStatus>((ref) async {
      ...
      final notificationsEnabled =
          await EnhancedNotificationService.areNotificationsEnabled();
...
class CampgroundActionsIOSCompatible {
  ...
  Future<void> _startMonitoring(Campground campground) async {
    try {
      ...
      final monitoredCount = DemoDataProvider.getMonitoredCampgrounds().length;
      if (monitoredCount == 1) {
        // First campground being monitored
        await AvailabilityMonitoringService.startMonitoring();
  ...
  Future<void> _stopMonitoring(Campground campground) async {
    try {
      ...
      final monitoredCount = DemoDataProvider.getMonitoredCampgrounds().length;
      if (monitoredCount == 0) {
        // No more campgrounds being monitored
        await AvailabilityMonitoringService.stopMonitoring();
